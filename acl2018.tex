...
Considerable research efforts have been devoted so far to make conversational 
agents smarter. 
One of the approaches is integrating user/speaker model into dialog systems.
In this way, an agent can learn abilities like: choosing the best response 
according to the specific user preference \cite{DBLP:journals/corr/JoshiMF17};
or using appropriate speech style based on profile or attributes of who 
it is interacting with \cite{herzig-EtAl:2017:INLG2017}.

A first attempt to model persona is from \newcite{li-EtAl:2016:P16-13} , 
who propose to assign specific personality and conversation style to agents 
based on learned persona embeddings.
Recent work by \newcite{ficler-goldberg:2017:StyVa} perform a conditional 
language model to generate text with stylistic restrictions.
\newcite{Qian2017AssigningPT} give the system an identity with profile
in order to enhance the response consistency.
Other researchers have recently attempted to introduce personalized information 
to dialog system by transfer learning 
\cite{Yang:2017:PRG:3077136.3080706, DBLP:journals/corr/ZhangLWZ17}.
